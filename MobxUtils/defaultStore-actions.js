...
function checkServerPath(pathList) {
  if (
    ...
    pathList.some((path) => typeof path !== "string" || path === "")
  ) {
    return false;
  }
...
async function querySystemMessageAsync(optionsBag) {
  try {
    const uri = getServerApiUri({
      defaultStore: this,
      routeId: "getSystemMessage",
    });

    const data = await queryServerApiAsync({
      method: "get",
      uri,
      ...optionsBag,
    });
