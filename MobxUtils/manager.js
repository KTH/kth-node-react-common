...
function createStoreManager(storeId, initCallback) {
  ...
  if (manager == null) {
    Global.managerInstances[storeId] = new MobxStoreManager(
      storeId,
      initCallback
    );
  ...
  class MobxStoreManager {
    constructor(storeId, initCallback) {
      if (typeof initCallback !== "function") {
        throw new Error(
          "<MobxStoreProvider/> failed internally - MobxStoreManager() got invalid arguments"
        );
      }
    ...
    initWithNewData(input) {
      if (isNoObject(input)) {
        throw new Error(
          "<MobxStoreProvider/> failed internally - initWithNewData() got invalid input"
        );
      }
    ...
    _ensureIsActive() {
      ...
      throw new Error(
        "<MobxStoreProvider/> failed internally - expected store to be initialized, already"
      );
    }
  }
