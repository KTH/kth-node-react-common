...
function List(props) {
  ...
  if (Array.isArray(allItems) === false || allItems.some(isNoObject)) {
    // eslint-disable-next-line no-console
    console.error(
      `<List basePath="${basePath}" /> failed - missing or invalid items`
    );
    return null;
  }
  ...
  controls.forEach((controlSetup) => {
    ...
    allSubPathsAndValues[subPath] = allItems.map((object) =>
      findInObject({ object, path: subPath })
    );
  });
  ...
  const _setCurrTargetPos = (newPos) => {
    if (
      rearrangeState.mode === "active" &&
      rearrangeState.currTargetPos !== newPos
    ) {
  ...
  for (let pos = 0; pos < allItems.length; pos++) {
    reorderedElements.push(
      <div
        key={pos}
        className="List-mouse-follower"
        onMouseEnter={() => _setCurrTargetPos(pos)}
      >
  ...
  const isListExtendable =
    typeof extendable === "function"
      ? extendable({ allItems })
      : extendable === true;
  const extendRow = isListExtendable ? (
    <div className="List-extend">
      <button
        ...
        onClick={() =>
          _appendItem({ dataBag, basePath, createItem, forceRefresh })
        }
      >
  ...
  return (
    <div
      ...
      onMouseLeave={() => _setCurrTargetPos(null)}
    >
      {reorderedElements}
      {extendRow}
    </div>
...
function _ensureValidPropsOnce(props) {
  ...
  try {
    ...
    if (
      itemKeyPath != null &&
      (typeof itemKeyPath !== "string" || itemKeyPath === "")
    ) {
...
function _getRearrangeControl({
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      className="List-item-rearrange"
      onMouseDown={_onMouseDown}
    >
      <img
        src={`${iconDirectory}arrow-up-down-white.svg`}
        alt="Ändra position"
      />
    </div>
  );
...
function _getRemoveControl({
  ...
  if (isItemRemovable) {
    const remove = () =>
      _removeItem({ dataBag, basePath, itemIndex, forceRefresh });
    return (
  ...
  return typeof removable === "function" ? (
    <div className="List-item-noremove" />
  ) : null;
}
