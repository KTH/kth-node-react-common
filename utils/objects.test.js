...
describe('Utility-module "objects" exports', () => {
  ...
  _runTestsAboutIsArrayOfObjects()
...
function _runTestsAboutIsArrayOfObjects() {
  describe('a function isArrayOfObjects() which', () => {
    const workingInput = _listTestData('valid-objects')
    const failingInput = _listTestData('non-objects')

    it('is accessible', () => {
      expect(isArrayOfObjects).toBeFunction()
    })

    it('- when given an empty array - returns true', () => {
      expect(isArrayOfObjects([])).toBeTrue()
    })

    it('- when given an array with one object, array or class instance - returns true', () => {
      workingInput.forEach(input => {
        expect([isArrayOfObjects([input]), input]).toEqual([true, input])
      })
    })

    it('- when given an array with several objects or the like - returns true', () => {
      expect(isArrayOfObjects([workingInput[0], workingInput[1], workingInput[2]])).toBeTrue()
    })

    it('- when given a non-array - returns false', () => {
      const nonArrays = [...workingInput, ...failingInput].filter(item => Array.isArray(item) === false)
      nonArrays.forEach(input => {
        expect([isArrayOfObjects(input), input]).toEqual([false, input])
      })
    })

    it('- when given an array with one non-object - returns false', () => {
      failingInput.forEach(input => {
        expect([isArrayOfObjects([input]), input]).toEqual([false, input])
      })
    })

    it('- when given an array with several non-objects - returns false', () => {
      expect(isArrayOfObjects([failingInput[0], failingInput[1], failingInput[2]])).toBeFalse()
    })

    it('- when given an array with a mixture of objects and non-objects - returns false', () => {
      expect(isArrayOfObjects([workingInput[0], workingInput[1], failingInput[0], failingInput[1]])).toBeFalse()
    })
  })
}
